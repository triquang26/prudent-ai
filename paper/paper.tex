...
\begin{abstract}
Practitioners size AI deployments against public evaluation evidence, assuming the open problem is \emph{estimate quality}. We ask a prior question: given the evidence that exists, is the decision \emph{identified at all}? We formalize \emph{evidence-decidability} and measure it over 1{,}716 right-sizing queries from documented production deployments. The criterion-robust finding: under any bounded belief about the measurable axes, \textbf{56.9\%} of decisions are underdetermined---\textbf{47.3 percentage points} (83\%) of this share blocked by axes no evaluation source measures anywhere, which admit no bounded range because there is no evidence to bound them with. Granting the contested declared-implies-binding assumption lifts this to \textbf{75.1\%--91.1\%} (skeptic-robust floor to full-domain upper); 56.9\% is the criterion-robust core. The mismatch has teeth: the automated-tooling default---ranking on visible axes alone---violates the hidden binding constraint on \textbf{57.1\%} of decisions (CI [46.8, 67.6]); imputation does not repair it, and a calibrated-transfer interval recovers only a guaranteed minority, \emph{certifying} the rest require measurement. That governance binds is measured, not merely declared: \textbf{34.5\%} of US-federal GenAI use-cases carry a structured binding field recorded independently of the configuration. The selective procedure abstains, names the blocking measurement, and---closing the loop with real acquisition---reaches full coverage with zero violations.
\end{abstract}
...
\looseness=-1 \textbf{The headline, scoped.} We report a range because the declared-implies-binding assumption is contestable. \emph{Criterion-robust core (\textbf{56.9\%}):} under any bounded belief about the measurable axes, 56.9\% are underdetermined; \textbf{47.3 pp} (83\%) of this share are blocked by axes no source measures anywhere---governance, reviewer burden, hardware---which admit no bounded range, there being no evidence to bound them with. \emph{Skeptic-robust floor (\textbf{75.1\%}):} deleting governance and reviewer-burden bindings leaves 75.1\% underdetermined (floor decomposition in Appendix~\ref{app:circularity_detail})---no declared-implies-binding assumption required. \emph{Full-domain upper bound (\textbf{91.1\%}):} granting bindings at face value gives 91.1\% (CI [89.7, 92.4]), but requires declared-implies-binding (measured at 0.1\%--34.5\%), so 91.1\% is the contested upper. Among underdetermined queries: 82.5\% have at least one measurable co-location failure; 17.5\% are structural-only; they overlap (62.0\% mixed; cross-tab in Appendix~\ref{app:circularity_detail}). Modal blockers: cost (1{,}289), governance (954), reviewer burden (751). \emph{Governance binding is measured, not only declared:} \textbf{34.5\%} of 1{,}290 US-federal GenAI use-cases carry a structured binding field (ATO/PII/high-impact) recorded \emph{independently of the configuration} (Appendix~\ref{app:binding_rate})---schema fields, not our tag mapping, so free of self-selection and the declared-implies-binding assumption.

\looseness=-1 \textbf{Robustness} (full ablations in Appendix~\ref{app:ablations}).
The result is not a knob artifact (verdict bit-identical under grounding percentiles p25--p75 and cost-tie tolerances to 20\%; full ablations in Appendix~\ref{app:reviewer}). (i) \emph{Confidence policy:} underdetermination stays $\ge 89.3\%$ across every filter; measured-only drives it to 100\%. (ii) \emph{Taxonomy:} deleting any single tag-to-axis mapping leaves the headline $\ge 83.2\%$. (iii) \emph{Query distribution:} a uniform prior yields \textbf{98.5\%}, an adversarial governance-light prior \textbf{95.7\%}, and a negative-control prior demanding only measured axes collapses to \textbf{12.4\%}---the cause is demand for unmeasured axes, not the corpus. (iv) \emph{Scale:} 20 sources leave the blind spot intact (Section~\ref{sec:substrate}). (v) \emph{Correlated overcounting:} deleting the governance \emph{and} reviewer-burden mappings together leaves \textbf{75.1\%} (75.9\% restricting governance to the 73 keyword-explicit cases), cost the modal blocker; and deleting whole industries (even all five regulated ones) leaves $\ge 88.8\%$ (Appendix~\ref{app:reviewer}). (vi) \emph{Declared-vs-binding:} binding each declared constraint only with probability $p$ gives a monotone \textbf{75.1\%}--\textbf{91.1\%}, still \textbf{84.3\%} at $p{=}0.5$. (vii) \emph{Independent corpora:} the headline replicates on two further separately-curated corpora (Evidently \textbf{96.4\%}; US Federal AI inventory \textbf{97.8\%}), the blind-spot share scaling with governance demand from $17.5\%$ to $72.4\%$ (Figure~\ref{fig:extval}).
...
\textbf{Calibrated transfer cures the curable part; the loop closes the rest} (Appendix~\ref{app:transfer}). A co-location failure is on a \emph{measurable} axis, so instead of a blind point fill we feed the verdict a \emph{distribution-free interval} predicted by cross-benchmark transfer and commit only when it does not straddle---inheriting $P(\text{infeasible}\mid\text{commit})\le\alpha$ and recovering \textbf{24.9\%} of masked-quality decisions at \emph{zero} violations (vs.\ 53.3\% for the point imputer), while the rest abstain \emph{certified} to need measurement and the never-measured axes \emph{refuse} an interval by construction. What transfer cannot recover the loop resolves---abstain, acquire the VoI-ranked axis \emph{for real}, re-decide---committing every decision with zero violations and minimum-sufficient cost in 2.0 probes; governance, lacking any measurement path, is the one place the loop refuses rather than fakes.
...
\looseness=-1 The map cannot by itself show that answering an underdetermined query is harmful. We test this where ground truth exists. RouterBench supplies per-prompt measured quality and cost for 11 models per benchmark; one benchmark at a time makes costs comparable. The slice algebra, fixed once: \textbf{31 slices $\times$ 17 queries = 527 masked decisions}, 30 with measured ground truth (510); a hidden axis \emph{bites} when at least one blind baseline violates the hidden truth---\textbf{28 of the 30} (476, the pooled set), the 2 no-bite slices controls, plus 1 medium-confidence function-calling slice (17). On each slice we bind on ground-truth axes, then \emph{hide one binding axis at the read level} (quality, which cheap trades off; cost stays visible): each rule sees an empty observation set for it, commits from visible evidence, and is scored against the hidden truth---a \emph{hidden violation} when the commitment violates the hidden constraint. Slice values score, never tune. Baselines span practice: \textbf{B1} accuracy-only; \textbf{B2} \emph{observed-Pareto} (rank on visible axes, missing treated satisfied)---the automated-tooling default; \textbf{B3} \emph{imputation} (median fill, then B2); \textbf{B4} missing-as-fail (treats every unknown as a constraint violation---never deciding, zero coverage); \textbf{B5} oracle (sees the hidden axis); \textbf{B6} a cost--quality frontier \citep{chen2023frugalgpt}.
...
\looseness=-1 Three readings matter. \emph{Imputation is a no-op}: B3 equals B2 on every slice (a constant fill orders nothing \citep{manski2003partial}); a learned imputer only halves the rate (30.6\%) and the strongest Bayesian rule reaches 4.7\% only by over-provisioning $70\times$, minimum-sufficient on \emph{none} of its commitments (Appendix~\ref{app:reviewer}). \emph{Feasible configs exist} (oracle B5: zero violations everywhere), and \emph{the bite is causal}: violations track Pareto-certified binding exactly (zero off-diagonal over 544 slices; Proposition~\ref{prop:char}). Masking the objective (cost) gives the same picture (Appendix~\ref{app:reviewer}).

\looseness=-1 \textbf{What links the two halves} is the binding mechanism, not a shared number: by Proposition~\ref{prop:char} an unmeasured binding axis both renders a query underdetermined (the map) and, where measurable, makes blind commitment violate (this section). \textbf{57.1\% is a foil}, not the conclusion: the content is the remedy comparison---imputation a no-op, learned imputer halves it, Bayesian/CC escape only by $70{\times}$--$127{\times}$ over-provisioning, the selective procedure alone achieving both zero violations and nonzero coverage. We do \emph{not} claim 57.1\% transfers to governance. The governance contribution has three layers: (i) a \emph{decidability claim}---governance, a property of (config, jurisdiction, use-case), is absent from every configuration-level source by construction, so any governance-binding decision is structurally underdetermined; (ii) a \emph{mechanism demonstration}---harm operates on quality (57.1\%) and on a governance-shaped categorical proxy (masking a hard self-hostability gate violates it on \textbf{16.7\%}--\textbf{53.3\%} of decisions; Appendix~\ref{app:reviewer}); (iii) a \emph{governance harm rate} that is unknowable, no public dataset pairing configurations with governance verdicts (Appendix~\ref{app:audit_data}). Nothing requires (iii); (i) and (ii) are the contribution.
...
Both free-text estimates are below the OMB anchor ($\hat{p}_{\text{OMB}} = 34.5\%$), which measures binding from self-reported structured fields rather than narrative prose.
...
\emph{Honest scope.} This is \emph{partial} recovery. On the full 30-benchmark cell set the transferable signal is heavy-tailed (the additive predictor's $R^2$ is $0.81$ but a cluster of benchmarks---including MMLU, ARC-challenge, HellaSwag---is near-unpredictable cross-benchmark), so a distribution-free interval tight enough to guarantee $\le5\%$ risk is wide; the $24.9\%$ recovered are decisions where a strong model sits far from the threshold. The $\sim$25\% is a \emph{structural} ceiling, not an artifact of loose intervals: a locally-adaptive (residual-normalized) split-conformal that tightens intervals and commits $32\%$ more held-out cells at $\alpha{=}0.05$---guarantee preserved (coverage $0.952$, feasibility error $0.022$)---leaves decision recovery \emph{unchanged} at $24.9\%$, because the limiter is the decision geometry (whether the cheapest contested config's interval resolves), not interval width. The contribution is therefore not ``transfer recovers most decisions''---it is that transfer recovers a \emph{guaranteed} minority safely and \emph{certifies the majority genuinely require measurement}, which is exactly the paper's thesis and the bridge to the loop.

\paragraph{The live acquisition loop, closed with real measurement.}
The simulated 2.0-vs-6.0 acquisition count (Section~\ref{sec:acq}) is here closed end-to-end: an underdetermined query triggers the VoI-ranked measurement, whose \emph{measured} value is revealed into the scratch overlay and the verdict re-runs, until commit. On 5 RouterBench slices (Table~\ref{tab:loop}) the loop commits every decision with \textbf{0} hidden violations and \textbf{100\%} minimum-sufficient cost, in \textbf{1.0} measurement when quality is hidden and \textbf{2.0} when quality and cost are both hidden. Governance has \emph{no} measurement path (Appendix~\ref{app:audit_data}), so when the only remaining blocker is governance the loop \emph{refuses}---raising rather than fabricating a value---and the frozen snapshot is byte-identical after every run.

\begin{table}[h]
\centering
\caption{\textbf{Live acquisition loop on 5 RouterBench slices.} Real measurement re-entered through the C7 interface; the frozen snapshot is unchanged; governance is refused.}
\label{tab:loop}
\footnotesize
\setlength{\tabcolsep}{4pt}
\begin{tabular}{lccccc}
\toprule
Setting & Cov. & Probes & Acq.\ cost & Hidden viol. & Min-suff. \\
\midrule
Mask quality (1 blocker) & 1.00 & 1.0 & 0.30 & 0 & 1.00 \\
Mask quality $+$ cost (2 blockers) & 1.00 & 2.0 & 0.35 & 0 & 1.00 \\
\bottomrule
\end{tabular}
\end{table}

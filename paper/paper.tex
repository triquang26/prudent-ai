...
\begin{figure}[t]
\centering
\includegraphics[width=0.74\linewidth]{figures/fig_teaser.pdf}
...
\looseness=-1 \textbf{Masking simulates missingness} (we hide measured axes; a live study is next, and on never-measured axes our claims are about decidability only); the \textbf{bite} is scoped to where the hidden axis binds and cheap trades it off (the no-bite control keeps it falsifiable); the \textbf{theory is anchored small} (a two-world family; the map computes verdicts independently of it). \textbf{The query prior is one corpus, with three external-validity layers}: (a) \emph{re-weighting}, bounded by the uniform (98.5\%) and adversarial (95.7\%) priors; (b) \emph{composition}, by whole-industry deletion ($\ge 88.8\%$, Appendix~\ref{app:reviewer}); (c) \emph{publication self-selection}---only companies that publish appear---which neither addresses and which needs a second independently-collected corpus (none on hand; MedHELM was access-gated), so we flag it as the key external-validity limitation, the likely bias favoring the finding (published deployers are plausibly \emph{more} governance-aware). A declared constraint need not bind---the 55.6\%/43.8\% are taxonomy outputs floored at $\sim$75\% (Appendix~\ref{app:reviewer}, which also details placement-invariant thresholds and ordering-only acquisition costs); the prescription is that decision rules know what they have not measured.

\looseness=-1 For most decisions in this corpus---three-quarters to nine-tenths, by how skeptically one reads declared requirements---\emph{the evidence does not determine the choice}; deciding anyway ships hidden violations, imputation does not repair them, the named measurement does. The finding is a diagnosis of the evaluation ecosystem: what gates deployment is not what public evaluation publishes. Evidence sufficiency, not estimate quality, is the binding constraint.

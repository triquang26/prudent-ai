...
\begin{figure}[t]
\centering
\includegraphics[width=0.72\linewidth]{figures/fig_teaser.pdf}
...
\textbf{The headline, scoped.} The number below is a property of \emph{this} corpus read under \emph{this} requirement taxonomy, not a constant of deployment in general; its skeptical floor (deleting the declared-governance requirements, below and Section~\ref{app:reviewer}) is \textbf{75.1\%}, resting then on cost, a measurable axis. The definition supplies the rigor (every rate a certified lower bound); the empirical content is the measured \emph{mismatch}---44--56\% requirement rates on axes reported by zero sources. Under the accuracy-only reading---what a leaderboard supports---\textbf{100\%} of these right-sizing queries are underdetermined. Granting the rule every axis the corpus measures lowers that only to \textbf{91.1\%} (1{,}563/1{,}716; 95\% CI [89.7, 92.4]). Decidability saturates at 8.9\% once cost and latency are visible; the three subsequent rungs (+throughput, +energy, full) are flat at that value. The ceiling is attribution, not noise, and \emph{structural} rather than a cost-co-location artifact: granting cost as determined---the cheaply-acquirable axis a skeptic rightly flags---resolves only \textbf{8.9\%} of decisions and leaves \textbf{82.2\%} underdetermined, \textbf{72.4\%} (95\% CI [70.3, 74.5]) on an axis unmeasured corpus-wide (16.0\% [14.3, 17.8] on such an axis \emph{alone}); no enrichment could decide these (Appendix~\ref{app:reviewer}). The 75.1\% floor, by contrast, \emph{does} lean on cost---granting it resolves 38\% there---so we treat 75.1\% as the skeptical bound and 91.1\%/72.4\% as load-bearing. More generous readings still leave most decisions open: binding only measurable axes gives 75.1\%, and a bounded-prior completion of missing measurable cells (versus the adversarial full domain) gives \textbf{56.9\%}, flat across bound widths \emph{because} the surviving core is dominated by the never-measured axes, which admit no bounded range at all (Appendix~\ref{app:reviewer}). The most frequent blockers are cost (1{,}289), governance (954), reviewer burden (751); the honest output is a \emph{set}---any config optimal under unmeasured values---and a single recommendation chooses arbitrarily among indistinguishable worlds (Section~\ref{sec:validation} measures the cost).

\textbf{Robustness} (full ablations in Appendix~\ref{app:ablations}).
The result is not a knob artifact. (i) \emph{Confidence policy:} underdetermination stays $\ge 89.3\%$ across every confidence filter; measured-only drives it to 100\%. (ii) \emph{Taxonomy:} deleting any single tag-to-axis mapping leaves the headline at $\ge 83.2\%$. (iii) \emph{Query distribution:} a uniform prior yields \textbf{98.5\%}; an adversarial prior that nearly never binds governance or reviewer burden still yields \textbf{95.7\%}; a negative-control prior demanding only measured axes collapses to 12.4\%---the cause is demand for unmeasured axes, not the particular corpus. (iv) \emph{Grounding sharpens, not flatters:} a uniform grid gives 85.7\% versus 91.1\%. (v) \emph{Scale:} 20 sources leave the blind spot intact (Section~\ref{sec:substrate}). (vi) \emph{Verdict mechanics:} bit-identical under grounding percentiles p25--p75 and cost-tie tolerances to 20\% (Appendix~\ref{app:reviewer}). (vii) \emph{Correlated overcounting:} deleting the governance \emph{and} reviewer-burden mappings together leaves \textbf{75.1\%} (87.3\% restricting governance to the 73 keyword-explicit cases; 75.9\% under both)---even the most skeptical reading keeps three quarters underdetermined, cost the modal blocker (Appendix~\ref{app:reviewer}). (viii) \emph{Corpus composition:} deleting whole industries (not down-weighting) leaves the headline $\ge 88.8\%$---even removing all five regulated industries, the governance drivers, with cost modal throughout (Appendix~\ref{app:reviewer}); this stresses \emph{which} deployments are indexed, distinct from publication self-selection (Section~\ref{sec:limitations}). (ix) \emph{Declared-vs-binding:} binding each declared governance/reviewer constraint only with probability $p$ gives a monotone \textbf{75.1\%} ($p{=}0$)--\textbf{91.1\%} ($p{=}1$), still \textbf{84.3\%} at $p{=}0.5$---the result does not need the strong declared-implies-binding assumption (Appendix~\ref{app:reviewer}).
...
Three readings of Table~\ref{tab:flagship} matter beyond the headline. First, \emph{median imputation is a no-op}: B3's violations are identical to B2's on every slice---a constant fill orders nothing and merely hides the violation, as the partial-identification view predicts \citep{manski2003partial}. A \emph{learned} imputer with real cross-context signal (leave-one-benchmark-out per-model mean) cuts the rate roughly in half---53.3\% to 30.6\%---yet still silently violates on a third of its commitments. The strongest probabilistic competitor---a \emph{Bayesian} expected-regret rule (per-model posterior over the hidden axis, minimizing expected feasibility-weighted regret)---drives violations to 4.7\% only by over-provisioning to a mean $70\times$ the cheapest-sufficient cost, minimum-sufficient on \emph{none} of its commitments (unchanged at quadrupled penalty). None recovers the cheapest sufficient configuration, and on the never-measured axes no signal exists to form any prior (Appendix~\ref{app:reviewer}). Second, \emph{feasible configurations exist}: the oracle B5 commits everywhere with zero violations, so the baselines fail for lack of evidence, not for lack of options. Third, \emph{the bite is causal, not declared}: violations track Pareto-certified binding exactly (287 bind-and-bite, 257 non-bind-and-no-bite, zero off-diagonal), and no rule violates on the control slice where cheap configurations satisfy the hidden axis---failure occurs precisely where the axis truly binds, as Proposition~\ref{prop:char} predicts. Masking the \emph{objective} instead (hide cost, the map's top blocker): the visible-axis rules degenerate to zero coverage, the leaderboard fallback hidden-violates 88.4\%, and the pipeline again closes full coverage at zero violations (Appendix~\ref{app:reviewer}).

\textbf{What links the two halves} is the binding mechanism, not a shared number: by Proposition~\ref{prop:char} an unmeasured binding axis both renders a query underdetermined (the map, over \emph{all} axes) and, where measurable, makes blind commitment violate (this section). We therefore do \emph{not} claim the 57.1\% transfers to governance---it is the measurable-axis instance of a corpus-wide failure mode; the governance claim stays one of decidability, unfalsifiable on the harm axis precisely because the axis is unmeasured. As a partial bridge, masking a hard \emph{categorical} self-hostability gate and committing blind violates it on \textbf{16.7\%} of decisions---to \textbf{53.3\%} where a high quality floor forces a proprietary model---the first harm test on a governance-shaped categorical constraint, grounded in the models' real licenses (Appendix~\ref{app:reviewer}). The equal-coverage win is \emph{measure-versus-guess} by construction (McNemar $272/0$ near-definitional); the load-bearing results are that the procedure \emph{knows when} to measure---the acquisition efficiency and held-out risk control of Sections~\ref{sec:acq}--\ref{sec:method}---not the gap itself.
...
\looseness=-1 Zero violations at zero coverage would be trivial (B4 achieves it mutely); the value is what the refusal \emph{returns}. With \textbf{two genuine blockers} (hiding quality \emph{and} cost on the same 510 decisions, neither sufficient alone), the ranked plan resolves every decision in \textbf{2.0 measurements at cost 0.35} versus \textbf{6.0 and 2.06} unplanned, every commitment feasible and minimum-sufficient (Appendix~\ref{app:reviewer}).


\subsection{Risk-tolerant operation: the guarantee transfers to held-out truth}

\looseness=-1 Operations may prefer a tolerated risk. The calibrated-margin variant scores against \emph{measured} ground truth (full-sample quality/cost as truth, a noisy 64-prompt subsample read, the margin calibrated on a disjoint half, 480/480). The calibration transfers (Figure~\ref{fig:anchors}, right): test feasibility risk \textbf{4.7\%} at \textbf{89.0\%} coverage for $\alpha{=}0.05$ (8.1\% at 92.7\% for $\alpha{=}0.10$). Two honest-boundary caveats---the margin controls \emph{feasibility} risk only, and a held-out prompt split (realized violations $36.6\%\!\to\!14.4\%/3.8\%$)---are in Appendix~\ref{app:prospective}.

\section{Limitations and Conclusion}
\label{sec:limitations}

\looseness=-1 \textbf{Scope---what this paper does and does not claim.} \emph{Masking simulates missingness} (a live study is next; on never-measured axes our claims are about decidability only); \emph{declared need not bind} (headline $\ge 84.3\%$ even if only half of declared constraints bind, \S\ref{sec:map}); and the single-corpus prior \emph{replicates on a fully independent second corpus} (Evidently AI, 502 deployments): \textbf{96.4\%} underdetermined, blind-spot share corpus-dependent (Appendix~\ref{app:reviewer}). Publication self-selection is the residual caveat.

\looseness=-1 For most decisions in this corpus---\textbf{56.9\%} even under bounded beliefs---\emph{the evidence does not determine the choice}, and deciding anyway ships violations the named measurement prevents: evidence sufficiency, not estimate quality, is the binding constraint on deployment.
...
\paragraph{External-validity replication on an independent second corpus.}
To test the strongest external-validity worry---publication self-selection in a single corpus---we replicate on a \emph{separately curated} corpus: the Evidently AI ML/LLM system-design database (MIT-licensed mirror, commit pinned), built from engineering blogs by a different maintainer with a different selection method than ZenML. From its \textbf{502} LLM deployments we derive queries under the \emph{same} semantics (universal quality, an industry governance rule, and a use-case$\to$axis map mirroring our tag rules; the cross-vocabulary map is documented and deliberately conservative) and run the \emph{identical} classifier against the \emph{same} frozen substrate. The headline replicates and slightly exceeds: \textbf{96.4\%} of decisions are underdetermined (vs.\ 91.1\%), confirming that most right-sizing decisions are unidentified is not an artifact of the ZenML corpus. Honestly, the \emph{composition} of the blind spot is corpus-dependent: this corpus is consumer-tech-skewed (governance declared in 13.8\% of deployments, reviewer burden 4.4\%, vs.\ 55.6\%/43.8\% in corpus 1), so its underdetermination is driven more by cost (a measurable-but-absent axis, the modal blocker on all 502) and less by never-measured governance (\textbf{17.5\%} blind-spot-attributable vs.\ 72.4\%). This is exactly the mechanism the industry-deletion and binding-probability sweeps predict---the headline is robust across corpora, while the governance-attributed fraction scales with how many regulated deployments the corpus contains. A tech-skewed independent corpus showing \emph{higher} underdetermination is evidence the finding is not inflated by corpus~1's composition.
...
Independent-corpus replication (App.) & 502 deployments & headline $96.4\%$ on a second corpus (W4) \\

...
The measurement makes the mismatch quantitative (Section~\ref{sec:map}), and Figure~\ref{fig:teaser} (left) shows it in one view. The finding nests by assumption strength. A \textbf{41.0\%} core is robust to \emph{both} the completion semantics and the declared-implies-binding assumption. Under any bounded belief about measurable axes but granting declared governance, \textbf{56.9\%} are underdetermined, \textbf{47.3 pp} of them on axes no source measures (governance, reviewer burden, hardware) that admit no bounded range; granting in addition that declared constraints bind brackets \textbf{75.1\%}--\textbf{91.1\%} (direct binding measurements: 0.1\%--34.5\%). The picture survives every confidence-filter, single-tag deletion ($\ge 83.2\%$), adversarial re-weighting ($\ge 95.7\%$), corpus scaling, and three independent-corpus replications.
...
\item \textbf{To our knowledge, the first decidability map over realistically distributed deployment queries}, reported as a triple nested by assumption strength: \textbf{41.0\%} robust to both the completion semantics and the declared-implies-binding assumption, \textbf{56.9\%} bounding the measurable axes, and \textbf{75.1\%--91.1\%} granting that declared constraints bind (direct measurements: 0.1\%--34.5\%). The blind spot is \textbf{82.5\%} co-location failures on measurable axes and \textbf{17.5\%} structural absences, of which only \textbf{20.5\%} are \emph{purely} measurable; robust to confidence policy, taxonomy, query distribution, corpus scale, and \emph{three} independent-corpus replications (Section~\ref{sec:map}).
...
\looseness=-1 The finding nests by assumption strength: a \textbf{41.0\%} core robust to every modelling choice, \textbf{56.9\%} bounding the measurable axes, and \textbf{75.1\%--91.1\%} granting that declared constraints bind. Deciding anyway ships violations only the named measurement prevents; evidence sufficiency, not estimate quality, is the binding constraint.

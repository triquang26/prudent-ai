...
\caption{\textbf{The decidability map over 1{,}716 real deployment queries} (full-domain completion, $p{=}1$---the \emph{adversarial ceiling} reading). Each bar fixes an evidence regime (the axes a rule may read) and shows the fraction of queries underdetermined (red) versus decidable (green); the bars plateau at the adversarial ceiling of 91.1\%. The \emph{certified floor of 41.0\%} (bounded completion, no declared binding; Table~\ref{tab:twobytwo}) is the irreducible core robust to every modelling choice and is what the claim leads with. Decidability improves only when an axis that actually binds enters the regime (cost, then latency) and \emph{plateaus at 8.9\% decidable} from the third rung onward, because the remaining queries bind axes the corpus does not measure---the empirical signature of Proposition~\ref{prop:char}.}
\label{fig:map}
\end{figure}
...
\looseness=-1 \textbf{Scope---what this paper does and does not claim.} \emph{Masking simulates missingness} (a live study is next; on never-measured axes our claims are about decidability only); \emph{declared need not bind}---but the certified floor (41.0\%) assumes \emph{none} does, and the rate stays $\ge 84.3\%$ even if half bind (\S\ref{sec:map}); and \emph{publication self-selection}---one corpus, though the headline replicates on two further independent ones (\S\ref{sec:map})---is the residual external-validity caveat.

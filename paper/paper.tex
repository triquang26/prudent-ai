...
\looseness=-1 \textbf{Scope---what this paper does and does not claim.} \emph{Masking simulates missingness} (a live study is next; on never-measured axes our claims are about decidability only); \emph{declared need not bind} (headline $\ge 84.3\%$ even if half bind, \S\ref{sec:map}); and the headline \emph{replicates on two further independent corpora}, the blind-spot share bracketing with governance demand (Appendix~\ref{app:reviewer}). Publication self-selection is the residual caveat.
...
\begin{figure}[h]
\centering
\includegraphics[width=0.95\linewidth]{figures/fig_external_validity.pdf}
\caption{\textbf{The headline replicates across three independently-curated corpora; the blind spot scales with demand for never-measured axes.} \textbf{Left:} on ZenML (mixed industry), Evidently (consumer tech), and the US Federal AI inventory (government), the underdetermination headline is \emph{robust} ($91.1/96.4/97.8\%$, blue), while the fraction blocked by a never-measured axis (red) varies with the corpus ($72.4/17.5/69.2\%$). \textbf{Right:} that blind-spot share is a near-linear function of how often the corpus declares governance or reviewer burden---low for consumer tech, high for government---so the variation is the mechanism (demand for unmeasured axes), not a corpus artifact. The government corpus independently recovers corpus~1's $72.4\%$ on a wholly different population.}
\label{fig:extval}
\end{figure}

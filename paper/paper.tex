...
\item \textbf{To our knowledge, the first decidability map over realistically distributed deployment queries}: underdetermination spans [56.9\%, 91.1\%] (skeptic-robust floor \textbf{75.1\%}; upper bound requires the contested declared-implies-binding assumption, measured at 0.1\%--34.5\%). The blind spot has two layers: \textbf{82.5\%} \emph{co-location failures}, where a measured axis (cost, latency, quality) has evidence in the corpus but not for the configuration that blocks the query, and \textbf{17.5\%} structural absences on never-measured axes; robust to confidence policy, taxonomy, query distribution, corpus scale, and \emph{three} independent-corpus replications (Section~\ref{sec:map}).
...
The price of violating the necessary condition is quantitative. On the canonical two-candidate family ($x_{\mathrm{cheap}}$ at cost $1$, $x_{\mathrm{safe}}$ at cost $1{+}\delta$, with one binding axis $a^*\notin\cl(R)$ unmeasured for $x_{\mathrm{cheap}}$ and satisfied by $x_{\mathrm{safe}}$), under the loss $L(\hat{x}) = \max(0, \mathrm{cost}(\hat{x}) - \mathrm{cost}(x^*)) + \lambda \mathbf{1}[\hat{x}\text{ violates}]$ (cost overshoot plus a violation penalty $\lambda$):
...
\end{theorem}
Abstention escapes the floor only by paying for a measurement, which the corollary prices exactly.
...
\looseness=-1 \textbf{The headline, scoped.} We report a range because the declared-implies-binding assumption is contestable. Under any bounded belief about the measurable axes, \textbf{56.9\%} of decisions are underdetermined, and \textbf{47.3} of those points (83\%) are blocked by axes no source measures anywhere (governance, reviewer burden, hardware), which admit no bounded range. Deleting the governance and reviewer-burden bindings entirely still leaves a \textbf{75.1\%} skeptic-robust floor with no binding assumption, while granting them at face value gives the \textbf{91.1\%} full-domain upper bound, contested because direct binding measurements run only 0.1\%--34.5\%. Among underdetermined queries, 82.5\% have a measurable co-location failure and 17.5\% are structural-only, overlapping in a 62.0\% mixed bucket (Appendix~\ref{app:circularity_detail}). Governance binding is moreover \emph{measured}, not only declared: \textbf{34.5\%} of 1{,}290 US-federal GenAI use-cases carry a structured binding field recorded independently of the configuration (Appendix~\ref{app:binding_rate}), free of self-selection and the declared-implies-binding assumption.

\looseness=-1 \textbf{Robustness} (full ablations in Appendix~\ref{app:ablations}).
The result is not a knob artifact (verdict bit-identical under grounding percentiles p25--p75 and cost-tie tolerances to 20\%; full ablations in Appendix~\ref{app:reviewer}). (i) \emph{Confidence policy:} underdetermination stays $\ge 89.3\%$ across every filter; measured-only drives it to 100\%. (ii) \emph{Taxonomy:} deleting any single tag-to-axis mapping leaves the headline $\ge 83.2\%$. (iii) \emph{Query distribution:} a uniform prior yields \textbf{98.5\%}, an adversarial governance-light prior \textbf{95.7\%}, and a negative-control prior demanding only measured axes collapses to \textbf{12.4\%}---the cause is demand for unmeasured axes, not the corpus. (iv) \emph{Correlated overcounting:} deleting the governance \emph{and} reviewer-burden mappings together leaves \textbf{75.1\%} (75.9\% restricting governance to the 73 keyword-explicit cases), cost the modal blocker; and deleting whole industries (even all five regulated ones) leaves $\ge 88.8\%$ (Appendix~\ref{app:reviewer}). (v) \emph{Declared-vs-binding:} binding each declared constraint only with probability $p$ gives a monotone \textbf{75.1\%}--\textbf{91.1\%}, still \textbf{84.3\%} at $p{=}0.5$. (vi) \emph{Independent corpora:} the headline replicates on two further separately-curated corpora (Evidently \textbf{96.4\%}; US Federal AI inventory \textbf{97.8\%}), the blind-spot share scaling with governance demand from $17.5\%$ to $72.4\%$ (Figure~\ref{fig:extval}).
...
\looseness=-1 \textbf{What links the two halves} is the binding mechanism, not a shared number. By Proposition~\ref{prop:char} an unmeasured binding axis both renders a query underdetermined (the map) and, where measurable, makes blind commitment violate (this section), so \textbf{57.1\% is a foil}: the content is the remedy comparison, where imputation is a no-op, a learned imputer halves it, Bayesian rules escape only by $70{\times}$--$127{\times}$ over-provisioning, and the selective procedure alone achieves both zero violations and nonzero coverage. We do \emph{not} claim 57.1\% transfers to governance. Governance is unmeasured in every configuration-level source by construction, so any governance-binding decision is structurally underdetermined (a decidability claim we verify directly), and the harm \emph{mechanism} is confirmed on quality and on a governance-shaped categorical gate (blind commitment violates it on \textbf{16.7\%}--\textbf{53.3\%} of decisions; Appendix~\ref{app:reviewer}). The governance harm \emph{rate} stays unknowable, since no public dataset pairs configurations with governance verdicts (Appendix~\ref{app:audit_data}), and nothing in our argument needs it.
...
Section~\ref{sec:validation} shows that committing blind is unsafe: a point imputer fills every $\bbot$ cell and hides a violation in 53.3\% of decisions. The open question is whether an underdetermined decision can be resolved \emph{without} always paying to measure. We answer it by sorting every underdetermined co-location decision into one of three classes, a partition we name the \textbf{resolution trichotomy} (Figure~\ref{fig:trichotomy}). A decision is \emph{curable by transfer} when its blocking axis is measurable and its value is predictable from the same model's evidence on its other benchmarks, so no new measurement is needed. It is \emph{resolvable by measurement} when the axis is measurable but not predictable, so its value must be physically acquired. It is \emph{structurally unmeasured} when no measurement path exists in any public source, as with governance, where there is no configuration-to-audit data to read. The two methods below handle the first two classes and refuse on the third.

\textbf{Calibrated transfer} cures the first class (Algorithm~\ref{alg:transfer}, Appendix~\ref{app:transfer}): for a fragmented-$\bbot$ quality cell it predicts a calibrated interval by leave-one-benchmark-out transfer, injects it as the cell's belief, runs the \emph{unchanged} verdict, and commits only when the interval lies entirely on the satisfying side of the threshold. Because the interval is a split-conformal set covering the truth with probability $\ge 1-\alpha$, this gives a finite-sample, distribution-free guarantee $P(\text{infeasible}\mid\text{commit})\le\alpha$, and on the structurally-unmeasured axes the predictor refuses an interval rather than paper over the blind spot. \textbf{The live acquisition loop} resolves the second class (Algorithm~\ref{alg:loop}): on an underdetermined query it commits if decidable, otherwise acquires the top $\VoI$-per-cost \emph{measurable} axis for real, writes the value to a scratch overlay that never touches the frozen store, and re-decides; when only structurally-unmeasured blockers remain it refuses rather than fabricate a value.
...
\paragraph{Method.}
A fragmented-$\bbot$ quality cell has evidence \emph{somewhere} in the corpus but not for the blocking configuration. Rather than fill it with a point (which commits blind), we predict a \emph{calibrated interval} by transfer from the same model's other benchmarks: a leave-one-benchmark-out two-way additive predictor $\hat{q}[m,B] = \text{coleff}[B] + \text{rowresid}[m]$ (benchmark difficulty from other models, model skill from its other benchmarks; no leakage of the target cell), wrapped in a split-conformal radius $\tau_\alpha$ calibrated on a seeded half of the cells. The interval $[\hat{q}-\tau_\alpha,\ \hat{q}+\tau_\alpha]$ is injected as the belief for that one $\bbot$ cell---via an overlay that synthesizes two observations so the existing $\varphi$ derives $[\mathrm{lo},\mathrm{hi}]$, never writing the frozen store---and the \emph{unchanged} verdict then commits only if the interval lies entirely on the satisfying side of the threshold. On the never-measured structural axes (governance, reviewer burden, energy, throughput, memory/hw) the predictor \emph{refuses} to emit an interval (there is no cross-context signal to transfer), so the method abstains there by construction. Algorithm~\ref{alg:transfer} states the procedure.

\begin{algorithm}[h]
\caption{Calibrated Transfer}\label{alg:transfer}
\begin{algorithmic}[1]
\Statex \textbf{Calibration (once).} Given the RouterBench quality matrix $Q$ over models $m$ and benchmarks $B$, and miscoverage $\alpha$:
\For{each observed cell $(m,B)$}
  \State $\hat q[m,B] \gets \mathrm{colEffect}[B] + \mathrm{rowResidual}[m]$ \Comment{leave-one-benchmark-out additive fit}
\EndFor
\State split observed cells by a fixed seed into a calibration half $\mathcal{C}$ and a test half
\State $\tau_\alpha \gets$ the $\lceil(n{+}1)(1{-}\alpha)\rceil/n$ empirical quantile of $\{|q-\hat q|:\mathcal{C}\}$, $n=|\mathcal{C}|$
\Statex \textbf{Inference (per fragmented-$\bbot$ quality cell of a blocking config $x$).}
\If{the axis is structurally unmeasured}
  \State \textbf{refuse}: emit no interval \Comment{no cross-context signal to transfer}
\Else
  \State $I \gets [\,\hat q-\tau_\alpha,\ \hat q+\tau_\alpha\,]$ clipped to $[0,1]$
  \State inject $I$ as the belief for $(x,\text{quality})$ and run the existing verdict
  \State \textbf{commit} only if $I$ lies entirely on the satisfying side of the threshold
\EndIf
\end{algorithmic}
\end{algorithm}
...
Algorithm~\ref{alg:loop} states the loop.

\begin{algorithm}[h]
\caption{Live Acquisition Loop}\label{alg:loop}
\begin{algorithmic}[1]
\Require underdetermined query $q$, masked substrate $S$, oracle $\mathcal{O}(x,a)$ returning the measured value
\State $\text{overlay}\gets\varnothing$ \Comment{scratch beliefs; the frozen store $S$ is never written}
\Loop
  \State classify $q$ against $S$ under $\text{overlay}$
  \If{\textsc{Decidable}} \textbf{commit} cheapest config; \textbf{stop}
  \ElsIf{\textsc{Infeasible}} \textbf{stop}
  \EndIf
  \State $a \gets$ top $\VoI$-per-cost blocking axis that is measurable
  \If{no such $a$} \textbf{refuse}: only structural blockers remain
  \EndIf
  \State \textbf{for} each candidate $x$: $\ \text{overlay}[(x,a)]\gets\mathcal{O}(x,a)$ \Comment{acquire}
\EndLoop
\end{algorithmic}
\end{algorithm}

...
\begin{abstract}
Practitioners size AI deployments against public evaluation evidence, assuming the open problem is \emph{estimate quality}. We ask a prior question: given the evidence that exists, is the decision \emph{identified at all}? We formalize \emph{evidence-decidability} and measure it over 1{,}716 right-sizing queries from documented production deployments, reporting a triple nested by assumption strength. Robust to \emph{both} the completion semantics and the contested declared-implies-binding assumption, \textbf{41.0\%} of decisions are underdetermined by a measurable co-location gap alone. Granting that a declared governance constraint is real (but still bounding the measurable axes) yields \textbf{56.9\%}; granting in addition that every declared constraint binds at the optimum brackets \textbf{75.1\%--91.1\%}, with direct binding measurements running 0.1\%--34.5\%. The mismatch has teeth: the automated-tooling default---ranking on visible axes alone---violates the hidden binding constraint on \textbf{57.1\%} of decisions on the measurable quality axis (CI [46.8, 67.6]); imputation does not repair it, and a calibrated-transfer interval recovers only a guaranteed minority, \emph{certifying} the rest require measurement. Governance is \emph{declared in structured form}, not only free text: \textbf{34.5\%} of US-federal GenAI use-cases carry a binding-indicator field recorded independently of the configuration. The selective procedure abstains, names the blocking measurement, and---closing the loop with real acquisition---reaches full coverage with zero violations.
\end{abstract}
...
\looseness=-1 \textbf{The headline, scoped as a nested triple.} Two orthogonal choices set the headline---how a $\bbot$ measurable cell completes (full-domain or bounded), and whether a declared governance constraint is granted as binding ($p{=}1$ or deleted $p{=}0$)---and their $2{\times}2$ (Table~\ref{tab:twobytwo}, Appendix~\ref{app:reviewer}) nests three readings. Robust to \emph{both}, the assumption-minimal core is \textbf{41.0\%}: bounded completion with no declared binding still leaves that many underdetermined by a measurable co-location gap alone (0.5 pp structural). Granting declared governance lifts the bounded reading to \textbf{56.9\%}, of which \textbf{47.3} points fall on never-measured axes; granting in addition that every declared constraint binds brackets \textbf{75.1\%}--\textbf{91.1\%}, contested because direct binding measurements run only 0.1\%--34.5\%. Among underdetermined queries only \textbf{20.5\%} are \emph{purely} measurable (resolvable without governance data); the rest mix a co-location gap with a structural blocker (Appendix~\ref{app:circularity_detail}). Governance is moreover \emph{declared in structured form}, not only free text: \textbf{34.5\%} of US-federal GenAI use-cases carry a binding-indicator field recorded independently of the configuration (Appendix~\ref{app:binding_rate})---a \emph{declared} requirement, not activeness at the optimum (Appendix~\ref{app:audit_data}).
...
\paragraph{The two modelling knobs, as a $2\times2$.}
The headline is set by two orthogonal choices: the completion of a $\bbot$ cell on a \emph{measurable} axis (full-domain, each cell ranging over its entire admissible domain---the most adversarial reading---versus bounded to the corpus-observed range), and whether a \emph{declared} governance/reviewer-burden constraint is granted as binding ($p{=}1$) or deleted ($p{=}0$). Table~\ref{tab:twobytwo} gives all four corners; every cell is a witness-based lower bound within its semantics (the sense in which each rate is ``at least''). The assumption-minimal corner---bounded completion \emph{and} no declared binding---is \textbf{41.0\%}, stable across bound widths (point-$p50$, $[p25,p75]$, $[p10,p90]$ all give 41.0\%) and only 0.5 pp structural-attributable, so it is almost entirely a measurable co-location gap that survives both modelling choices. Each added assumption is then a quantified increment: bounding only the measurable axes costs the gap between the columns, and granting declared binding costs the gap between the rows.

\begin{table}[h]
\centering
\footnotesize
\caption{\textbf{The two modelling knobs as a $2\times2$ of underdetermination rates.} Rows: completion of a $\bbot$ measurable cell. Columns: declared governance/reviewer-burden binding granted ($p{=}1$) or deleted ($p{=}0$). The assumption-minimal corner (bounded, deleted) is robust to both choices.}
\label{tab:twobytwo}
\begin{tabular}{lcc}
\toprule
Completion of $\bbot$ measurable cell & binding granted ($p{=}1$) & bindings deleted ($p{=}0$) \\
\midrule
Full-domain (adversarial) & 91.1\% \emph{(published)} & 75.1\% \emph{(skeptic floor)} \\
Bounded (observed range) & 56.9\% \emph{(criterion-robust)} & \textbf{41.0\%} \emph{(assumption-minimal)} \\
\bottomrule
\end{tabular}
\end{table}

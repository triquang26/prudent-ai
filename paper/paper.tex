...
\begin{abstract}
Practitioners size AI deployments against public evaluation evidence, assuming the open problem is \emph{estimate quality}. We ask a prior question: given the evidence that exists, is the decision \emph{identified at all}? We formalize \emph{evidence-decidability} and measure it over 1{,}716 right-sizing queries from documented production deployments. The criterion-robust finding: under any bounded belief about the measurable axes, \textbf{56.9\%} of decisions are underdetermined---\textbf{47.3 percentage points} (83\%) of this share blocked by axes no evaluation source measures anywhere, which admit no bounded range because there is no evidence to bound them with. Granting the contested declared-implies-binding assumption lifts this to \textbf{75.1\%--91.1\%} (skeptic-robust floor to full-domain upper); 56.9\% is the criterion-robust core. The mismatch has teeth: a deployer who has not yet acquired the missing measurement, ranking on visible axes alone, violates the hidden binding constraint on \textbf{57.1\%} of decisions (CI [46.8, 67.6]); a learned imputer halves but does not repair the rate. The selective procedure abstains, names the blocking measurement, and reaches full coverage with zero violations---the only approach achieving both.
\end{abstract}
...
\looseness=-1 \textbf{The headline, scoped.} We report a range because the declared-implies-binding assumption is contestable. \emph{Criterion-robust core (\textbf{56.9\%}):} under any bounded belief about the measurable axes, 56.9\% are underdetermined; \textbf{47.3 pp} (83\%) of this share are blocked by axes no source measures anywhere---governance, reviewer burden, hardware---which admit no bounded range, there being no evidence to bound them with. \emph{Skeptic-robust floor (\textbf{75.1\%}):} deleting governance and reviewer-burden bindings leaves 75.1\% underdetermined (floor decomposition in Appendix~\ref{app:circularity_detail})---no declared-implies-binding assumption required. \emph{Full-domain upper bound (\textbf{91.1\%}):} granting bindings at face value gives 91.1\% (CI [89.7, 92.4]), but requires declared-implies-binding (measured at 0.1\%--34.5\%), so 91.1\% is the contested upper. Among underdetermined queries: 82.5\% have at least one measurable co-location failure; 17.5\% are structural-only; they overlap (62.0\% mixed; cross-tab in Appendix~\ref{app:circularity_detail}). Modal blockers: cost (1{,}289), governance (954), reviewer burden (751).
...
\looseness=-1 The map cannot by itself show that answering an underdetermined query is harmful. We test this where ground truth exists. RouterBench supplies per-prompt measured quality and cost for 11 models per benchmark; one benchmark at a time makes costs comparable. The slice algebra, fixed once: \textbf{31 slices $\times$ 17 queries = 527 masked decisions}, 30 with measured ground truth (510); a hidden axis \emph{bites} when at least one blind baseline violates the hidden truth---\textbf{28 of the 30} (476, the pooled set), the 2 no-bite slices controls, plus 1 medium-confidence function-calling slice (17). On each slice we bind on ground-truth axes, then \emph{hide one binding axis at the read level} (quality, which cheap trades off; cost stays visible): each rule sees an empty observation set for it, commits from visible evidence, and is scored against the hidden truth---a \emph{hidden violation} when the commitment violates the hidden constraint. Slice values score, never tune. Baselines span practice: \textbf{B1} accuracy-only; \textbf{B2} \emph{observed-Pareto} (rank on visible axes, missing treated satisfied)---the pre-measurement default; \textbf{B3} \emph{imputation} (median fill, then B2); \textbf{B4} missing-as-fail (treats every unknown as a constraint violation---never deciding, zero coverage); \textbf{B5} oracle (sees the hidden axis); \textbf{B6} a cost--quality frontier \citep{chen2023frugalgpt}.
...
\looseness=-1 The criterion-robust finding: under any bounded belief about the measurable axes, \textbf{56.9\%} of decisions are underdetermined---\textbf{47.3 pp} (83\%) blocked by axes no source measures, with the full-domain reading bracketing 75.1\%--91.1\%. Deciding anyway ships violations only the named measurement prevents; evidence sufficiency, not estimate quality, is the binding constraint.

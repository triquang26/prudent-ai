...
\looseness=-1 \textbf{Masking simulates missingness} (we hide measured axes; a live study is next; on never-measured axes our claims are about decidability only); the \textbf{bite} is scoped and graded by trade-off strength (Appendix~\ref{app:reviewer}); the \textbf{theory is anchored small} (a two-world family the map does not rely on). \textbf{The query prior is one corpus, with three external-validity layers}: (a) \emph{re-weighting}, bounded by the uniform (98.5\%) and adversarial (95.7\%) priors; (b) \emph{composition}, by whole-industry deletion ($\ge 88.8\%$, Appendix~\ref{app:reviewer}); (c) \emph{publication self-selection}---only companies that publish appear---needs a second independently-collected corpus (none on hand; MedHELM was access-gated): the key external-validity limitation, its bias plausibly favoring the finding (mechanism, Appendix~\ref{app:reviewer}). A declared constraint need not bind---the 55.6\%/43.8\% are taxonomy outputs floored at $\sim$75\% (Appendix~\ref{app:reviewer}); the prescription is that decision rules know what they have not measured.

...
\begin{abstract}
Practitioners size AI deployments against public evaluation evidence, assuming the open problem is \emph{estimate quality}. We ask a prior question: given the evidence that exists, is the decision \emph{identified at all}? We formalize \emph{evidence-decidability}: a decision is decidable iff one configuration is provably feasible and cheapest in every world consistent with the evidence. We measure it over right-sizing queries derived from 1{,}716 documented production LLM deployments, read against a provenance-tracked corpus of public results. The finding is a measured \emph{mismatch}: the properties that gate real deployments---governance (declared in \textbf{55.6\%} of them) and reviewer burden (\textbf{43.8\%})---are reported by \emph{no} public source. Its bookkeeping: most decisions are not identified even when a rule sees every measured axis---from \textbf{56.9\%} under any bounded belief to \textbf{91.1\%} under the conservative full-domain reading, structural not tuned. Granting cost as fully known still leaves \textbf{82\%} underdetermined, \textbf{72\%} of them on axes no source measures; deleting the declared-governance requirements outright still leaves \textbf{75.1\%}, and the headline replicates across three independently-curated corpora. The mismatch has teeth where ground truth lets us measure it: with one binding axis hidden, ranking on the visible axes silently violates the hidden constraint, point imputation reproduces the violations, and a Bayesian rule avoids them only by over-provisioning away from the cheapest sufficient configuration. A selective procedure instead abstains and names the measurement that resolves the decision, ranking blockers by value per unit acquisition cost. We read the result as a \emph{diagnosis} of the evaluation ecosystem and a decision discipline that prices the next measurement, not a deployed tool: evidence sufficiency, not estimate quality, is the binding constraint on deployment decisions.
\end{abstract}
...
\item \textbf{The first decidability map over realistically distributed deployment queries}: 91.1\% of 1{,}716 decisions derived from documented deployments are underdetermined at full evidence (56.9\%--91.1\% across completion semantics); 72.4\% (95\% CI [70.3, 74.5]) are blocked by an axis no source measures at all; the blind spot is structural and robust to the confidence policy, the derivation taxonomy, the query distribution, corpus scale, and replication on \emph{three} independently-curated corpora (Section~\ref{sec:map}).
...
\textbf{The headline, scoped.} The number is a property of \emph{this} corpus and taxonomy, not a constant of deployment; the definition supplies the rigor (every rate a certified lower bound), the empirical content is the measured \emph{mismatch}---44--56\% requirement rates on axes reported by zero sources. Under the accuracy-only reading a leaderboard supports, \textbf{100\%} of these queries are underdetermined; granting every axis the corpus measures lowers that only to \textbf{91.1\%} (1{,}563/1{,}716; 95\% CI [89.7, 92.4]), and decidability saturates at 8.9\% once cost and latency are visible (Figure~\ref{fig:map}). The ceiling is \emph{structural}, not a cost-co-location artifact: granting cost as determined resolves only \textbf{8.9\%} and leaves \textbf{82.2\%} underdetermined, \textbf{72.4\%} (95\% CI [70.3, 74.5]) on an axis unmeasured corpus-wide; no enrichment could decide these. The skeptical floor (deleting declared-governance requirements) is \textbf{75.1\%}, which \emph{does} lean on cost, so we treat it as the floor and 91.1\%/72.4\% as load-bearing; a bounded-prior completion gives \textbf{56.9\%}, flat across bound widths because the surviving core is the never-measured axes, which admit no bounded range (Appendix~\ref{app:reviewer}). The modal blockers are cost (1{,}289), governance (954), reviewer burden (751); the honest output is a \emph{set}, and a single recommendation chooses arbitrarily among indistinguishable worlds.
...
The map cannot by itself show that answering an underdetermined query is harmful. We test this where ground truth exists. RouterBench supplies per-prompt measured quality and cost for 11 models per benchmark; one benchmark at a time makes costs comparable. The slice algebra, fixed once: \textbf{31 slices $\times$ 17 queries = 527 masked decisions}, 30 with measured ground truth (510); a hidden axis \emph{bites} when at least one blind baseline violates the hidden truth---\textbf{28 of the 30} (476, the pooled set), the 2 no-bite slices controls, plus 1 medium-confidence function-calling slice (17). On each slice we bind on ground-truth axes, then \emph{hide one binding axis at the read level} (quality, which cheap trades off; cost stays visible): each rule sees an empty observation set for it, commits from visible evidence, and is scored against the hidden truth---a \emph{hidden violation} when the commitment violates the hidden constraint. Slice values score, never tune. Baselines span practice: \textbf{B1} accuracy-only; \textbf{B2} \emph{observed-Pareto} (rank on visible axes, missing treated satisfied)---the honest default; \textbf{B3} \emph{imputation} (median fill, then B2); \textbf{B4} missing-as-fail; \textbf{B5} oracle (sees the hidden axis); \textbf{B6} a cost--quality frontier \citep{chen2023frugalgpt}.
...
\textbf{What links the two halves} is the binding mechanism, not a shared number: by Proposition~\ref{prop:char} an unmeasured binding axis both renders a query underdetermined (the map) and, where measurable, makes blind commitment violate (this section); we do \emph{not} claim the 57.1\% transfers to governance, which stays a decidability claim, unfalsifiable on the harm axis because the axis is unmeasured. As a partial bridge, masking a hard \emph{categorical} self-hostability gate and committing blind violates it on \textbf{16.7\%} of decisions---to \textbf{53.3\%} where a high quality floor forces a proprietary model---the first harm test on a governance-shaped constraint, grounded in real model licenses (Appendix~\ref{app:reviewer}). The equal-coverage win is \emph{measure-versus-guess} by construction (McNemar $272/0$ near-definitional); the load-bearing results are the acquisition efficiency and risk control of \S\S\ref{sec:acq}--\ref{sec:method}, not the gap.
...
\looseness=-1 For most decisions here---\textbf{56.9\%} even under bounded beliefs---\emph{the evidence does not determine the choice}; deciding anyway ships violations the named measurement prevents---evidence sufficiency, not estimate quality, binds.
...
\emph{Multi-gate robustness.} To rule out a single-gate artifact, we repeat the battery under two further independently-defined categorical gates over the same 150-instance pool: G2 (EU data-residency / GDPR: open-weights and GPT qualify; Claude models do not, lacking a documented Anthropic EU data centre as of the corpus date) yields HVR $= \mathbf{8.0\%}$; G3 (commercial fine-tuning rights: Apache-2.0 models only, i.e.\ Mistral-7B and Mixtral-8x7B) yields HVR $= \mathbf{45.3\%}$, the higher rate reflecting only two admissible models. Blind-commit harm holds across all three gates (min HVR $= 8.0\%$), ruling out the single-gate artifact concern. This is the first harm validation on hard categorical, governance-shaped constraints. The constraint and its admissibility set are real (data sovereignty is a documented enterprise requirement, and self-hostability is fixed by each model's license); what we supply is the decision to test \emph{this} governance constraint, since no source pairs configurations with audited governance outcomes.
...
Two external anchors bracket the plausible $p$ range. From \emph{LLM-ensemble annotation} (Qwen3-VL-8B-Instruct $\times$ 3 prompt variants, majority vote) over the 954 governance-tagged ZenML case studies, we estimate the declared$\to$binding rate at $\hat{p} = 7.97\%$ (95\% CI: 6.3\%--9.6\%; Fleiss $\kappa = 0.23$, fair agreement). From the \emph{US Federal AI Inventory} (OMB 2025, $n{=}1{,}290$ GenAI use cases), the fraction carrying at least one governance-binding indicator (high-impact rights/safety system, authority-to-operate required, or personal data involved) is $\hat{p}_{\text{OMB}} = 34.5\%$ (445/1{,}290). Together these yield a data-grounded range: at $p = 0.08$ the headline is $75.5\%$; at $p = 0.35$ it is $\approx82\%$---both within the published $75.1\%$--$91.1\%$ sensitivity envelope, and both well above the three-quarters floor. The $p$-sweep is retained as a sensitivity analysis; the measured anchors provide grounded reference points within it.
...
We ran LLM annotation over $n{=}954$ governance-tagged ZenML case studies using Qwen3-VL-8B-Instruct $\times$ 3 prompt variants (majority vote); the smaller Qwen2.5-0.5B model was also evaluated but proved degenerate (all 954 cases labelled UNDETERMINED) and is excluded from the primary estimate.
Labels are BINDING / NON\_BINDING / UNDETERMINED.
The primary 8B-model estimate: $\hat{p} = 7.97\%$ (76/954 BINDING; 95\% CI: 6.3\%--9.6\%; Fleiss $\kappa = 0.226$ across the 3 prompt variants, fair agreement).
Labels are LLM-derived, not human gold; as a floor check, the recall against 73 keyword-explicit anchor cases with confirmed candidate-excluding language is 17.8\% (13/73)---consistent with the keyword screen being a conservative lower bound on true BINDING cases.
The measured $\hat{p} = 7.97\%$ maps to the sensitivity sweep at $p \approx 0.08$, giving a headline of $75.5\%$ (Appendix~\ref{app:reviewer}), well within the $[75.1\%, 91.1\%]$ envelope.

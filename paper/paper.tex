...
\begin{abstract}
Practitioners size AI deployments against public evaluation evidence, assuming the open problem is \emph{estimate quality}. We ask a prior question: given the evidence that exists, is the decision \emph{identified at all}? We formalize \emph{evidence-decidability} and measure it over 1{,}716 right-sizing queries from documented production deployments. The skeptic-robust finding: \textbf{75.1\%} of decisions are underdetermined without any governance-binding assumption---the defensible floor---decomposing as \textbf{51.4\%} cost-only (cheap: acquisition cost $\approx 0$), \textbf{43.2\%} cost+latency, and \textbf{5.4\%} cost+quality co-location failures. Of all 1{,}563 underdetermined decisions: \textbf{20.5\%} are purely-measurable (no structural axis in the blocking set); \textbf{62.0\%} are mixed (both co-location failure and structural absence); \textbf{17.5\%} are structural-only. The 82.5\% ``co-location failures'' and 72.4\% ``structural'' figures are not mutually exclusive; the 62.0\% overlap is the mixed bucket. Under the full governance-binding assumption the rate rises to \textbf{91.1\%}, but direct binding-rate measurements (0.1\%--34.5\%) make this a contested upper bound; \textbf{75.1\%} is the defensible number. The mismatch has teeth: imputation is a no-op (median fill leaves the 57.1\% violation rate unchanged); a learned imputer halves it (30.6\%); Bayesian/CC rules escape only by $70{\times}$--$127{\times}$ over-provisioning. The selective procedure abstains and names the blocking measurement, reaching full coverage with zero violations---the only approach achieving both.
\end{abstract}
...
\looseness=-1 \textbf{The headline, scoped.} We report a range because the declared-implies-binding assumption is contestable. \emph{Skeptic-robust floor (\textbf{75.1\%}):} deleting governance and reviewer-burden bindings leaves 75.1\% underdetermined; the floor decomposes as \textbf{51.4\%} cost-only (cheap: $\approx\$0$), \textbf{43.2\%} cost+latency (moderate), \textbf{5.4\%} cost+quality (expensive, \$85--\$11k); granting cost resolves 38.2 pp, leaving a 1.2\% structural residual---no assumption required. \emph{The 82.5\% and 72.4\% are not mutually exclusive} (cross-tab in Appendix~\ref{app:circularity_detail}): \textbf{20.5\%} purely-measurable (no structural axis in blocking set), \textbf{62.0\%} mixed (co-location failure AND structural absence both present), \textbf{17.5\%} structural-only; 82.5\% counts $\ge 1$ measurable blocker; 72.4\% of all queries counts $\ge 1$ structural blocker; 62.0\% is the overlap. \emph{Full-domain upper bound (\textbf{91.1\%}):} granting bindings at face value gives 91.1\% (95\% CI [89.7, 92.4]), but requires declared-implies-binding (instruments: 0.1\%--34.5\%), so 91.1\% is the upper bound. Modal blockers: cost (1{,}289), governance (954), reviewer burden (751).

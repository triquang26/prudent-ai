...
\looseness=-1 \textbf{Scope---what this paper does and does not claim.} \emph{Masking simulates missingness} (we hide measured axes; a live study is next; on never-measured axes our claims are about decidability only); the \emph{bite} is scoped and graded by trade-off strength (Appendix~\ref{app:reviewer}); the \emph{theory is anchored small} (a two-world family the map does not rely on). The \emph{query prior is one corpus, with three external-validity layers}: (a) \emph{re-weighting}, bounded by the uniform (98.5\%) and adversarial (95.7\%) priors; (b) \emph{composition}, by whole-industry deletion ($\ge 88.8\%$, Appendix~\ref{app:reviewer}); (c) \emph{publication self-selection}---only companies that publish appear---needs a second independently-collected corpus (none on hand; MedHELM was access-gated): the key external-validity limitation, its bias plausibly favoring the finding (mechanism, Appendix~\ref{app:reviewer}). A declared constraint need not bind---the 55.6\%/43.8\% are taxonomy outputs floored at $\sim$75\% (Appendix~\ref{app:reviewer}); the prescription is that decision rules know what they have not measured.

\looseness=-1 For most decisions in this corpus---\textbf{56.9\%} even under bounded beliefs about the unmeasured axes, three-quarters to nine-tenths by how skeptically one reads declared requirements---\emph{the evidence does not determine the choice}; deciding anyway ships hidden violations, imputation does not repair them, the named measurement does. The finding is a diagnosis: what gates deployment is not what public evaluation publishes, and evidence sufficiency---not estimate quality---is the binding constraint.

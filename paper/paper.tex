...
\item \textbf{A selective procedure whose refusal is informative}: commit / abstain-and-name-the-measurement / infeasible, with (i) an acquisition plan that, under two genuine blockers, resolves decisions in 2.0 measurements versus 6.0 for unplanned probing, (ii) a numerically verified two-world identity tying the named measurement's VoI to the regret floor on a minimal family (a solver-precision check, not a general guarantee), and (iii) a feasibility guarantee that transfers to held-out measured ground truth (Sections~\ref{sec:method}, \ref{sec:validation}).
\end{enumerate}
...
\looseness=-1 Zero violations at zero coverage would be trivial (B4 achieves it mutely); the value is what the refusal \emph{returns}. The plan's primary test has \textbf{two genuine blockers}: hiding quality \emph{and} cost on the same 510 decisions (neither measurement alone resolves any query, by ablation), the ranked plan resolves every decision in \textbf{2.0 measurements at acquisition cost 0.35} versus \textbf{6.0 and 2.06} unplanned, every final commitment feasible and minimum-sufficient (Appendix~\ref{app:reviewer}).


\subsection{Risk-tolerant operation: the guarantee transfers to held-out truth}

\looseness=-1 Operations may prefer a tolerated risk. The calibrated-margin variant is scored against \emph{measured} ground truth: full-sample per-model quality and cost are the truth, the rule sees a noisy 64-prompt subsample, and the margin chosen on a calibration half applies to a disjoint test half (480/480). The calibration transfers (Figure~\ref{fig:anchors}, right): test feasibility risk \textbf{4.7\%} at \textbf{89.0\%} coverage for $\alpha{=}0.05$ (8.1\% at 92.7\% for $\alpha{=}0.10$). Two honest boundaries: the one-sided margin controls \emph{feasibility} risk only (the joint two-sided band: 12.7\% coverage), and a held-out prompt split cuts realized violations from 36.6\% to 14.4\%/3.8\% at $m{=}0.05/0.10$ (Appendix~\ref{app:prospective}).

\section{Limitations and Conclusion}
\label{sec:limitations}

\looseness=-1 \textbf{Scope---what this paper does and does not claim.} \emph{Masking simulates missingness} (a live study is next; on never-measured axes our claims are about decidability only); the \emph{bite} is graded and the \emph{theory anchored small} (a two-world family the map does not rely on; Appendix~\ref{app:reviewer}). \emph{Declared need not bind}: binding each declared governance/reviewer constraint with probability $p$ leaves the headline monotone $75.1$--$91.1\%$, still $84.3\%$ at $p{=}0.5$. The \emph{query prior is one corpus}---robust to re-weighting ($\ge 95.7\%$) and whole-industry deletion ($\ge 88.8\%$), but \emph{publication self-selection} needs a second corpus (none on hand): the key external-validity limitation, its bias plausibly favoring the finding (Appendix~\ref{app:reviewer}).

\looseness=-1 For most decisions in this corpus---\textbf{56.9\%} of them even under bounded beliefs---\emph{the evidence does not determine the choice}; deciding anyway ships violations, imputation does not repair them, the named measurement does. The finding is a diagnosis: what gates deployment is not what public evaluation publishes; evidence sufficiency, not estimate quality, is the binding constraint.

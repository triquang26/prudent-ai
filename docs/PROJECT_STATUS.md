...
**Update (node `w7kfzr`): full HELM-suite batch → 20 sources (§13 target met).** Ingested every viable text/LLM HELM suite (classic, mmlu, reasoning, finance, long-context, thaiexam, cleva, ewok, mmlu-winogrande-afr, capabilities, safety, air-bench, arabic-enterprise, torr). Substrate now **20 sources / 5164 obs / 655 configs / 19 τ**; quality range held [0,1] (off-scale jury ratings skipped, 0 corruption). **Representativeness finding:** over the full 20-source, multi-domain corpus the three hard-to-observe axes (memory_hw, governance, reviewer_burden) report **0 evidence — miss-rate 1.000**, and overall cell miss-rate is **0.728** — even the safety/AIR-Bench leaderboards report no governance (they measure model-safety, not deployment-governance). C1's structural blind-spot is corpus-robust, not a small-source artifact. Excluded per instruction: multimodal suites, HAL (401), BEIR (paper-extraction), and the κ-gated paper-row extraction (needs human annotators). See `docs/data_helm_suites.md`.

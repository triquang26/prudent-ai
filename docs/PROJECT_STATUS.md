...
| **C3** (method) | 3-state selective procedure + **VoI** + **coverage guarantee** — abstain *informatively* (name the field to measure), commit only when ≥1−α correct. | **DV4** (coverage–risk) + **DV5** (VoI-lift), P4 + P5 V2 | ✅ evidenced (proxy-truth) · 🟡 real-GT calibration open | VoI(a*)=Δ(R) exact; guarantee risk **0%** @ coverage **10.5%**; V2 VoI-pick commit-correct **1.0 vs 0.2** random | VoI=Δ(R) pinned rel_tol 1e-9 (4 pts); coverage–risk on proxy-truth (κ_truth=H+M+L, **not** held-out GT — W4); V2 lift is n=5 pilot (W11) |
...
| **DV5** VoI lift | regret reduction measuring top-VoI vs random | ✅ yes (pilot n=5) | V2: VoI-pick commit-correct **1.0** vs random **0.2**; P4 cost-aware VoI/cost lift 1.03× (raw 1.00× — ties on ⊥ axes) | `docs/P5_validation.md` §4; `docs/P4_selective_procedure.md` §3.3 |
